...
struct CaloDataPacket {
  CalorimeterHitReadoutPacket dataPacket;
  std::vector<CalorimeterHitReadoutPacket> hitPacketVec; //TODO - check this
...
void ArtBinaryPacketsFromDigisNew::printCalorimeterData(CaloDataPacket const& caloData) {
  CalorimeterHitReadoutPacket packet = caloData.dataPacket;
...
void ArtBinaryPacketsFromDigisNew::fillCalorimeterDataPacket(CaloDAQMap const& calodaqconds,
                                                          const CaloDigi& CD,
                                                          CalorimeterHitReadoutPacket& CaloData) {
  ...
  CaloData.BoardID             = globalROCID % number_of_calo_rocs_per_dtc;
  CaloData.ChannelStatusFlagsA = 0;
  CaloData.ChannelStatusFlagsB = 0;
  CaloData.unused              = 0;

  CaloData.ChannelNumber = DiracChannel;  // modified as it should be in the packet
  CaloData.DIRACA        = packetId; //Change-5
  CaloData.DIRACB        = (((CD.SiPMID() % 2) << 12) | (crystalId)); // this is useless for the moment .. can be a test
  CaloData.ErrorFlags    = 0;
  CaloData.Time          = CD.t0();
  std::vector<adc_t> theWaveform; //TODO check this works
  for (size_t i = 0; i < CD.waveform().size(); ++i) {
    theWaveform.push_back((adc_t)CD.waveform().at(i));
  }
  CaloData.NumberOfSamples           = theWaveform.size();
  CaloData.IndexOfMaxDigitizerSample = waveformMaximumIndex(theWaveform); 
...
void ArtBinaryPacketsFromDigisNew::addCaloHitToCaloPacket(calo_data_block_t& caloDataBlock,
                                                       CalorimeterHitReadoutPacket& caloHit) {
...
void ArtBinaryPacketsFromDigisNew::fillCalorimeterDataStream(DTCLib::DTC_Event& currentEvent,
  ...
  if (caloData.second.hitPacketVec.size() != 0) {
    sz += sizeof(CalorimeterHitReadoutPacket) + caloData.second.hitPacketVec.size() * sizeof(uint16_t);
...
void ArtBinaryPacketsFromDigisNew::fillCalorimeterHeaderDataPacket(CaloDAQMap const& calodaqconds,
  ...
  // Word 0
  adc_t nBytes =
      sizeof(DataBlockHeader) + sizeof(CalorimeterHitReadoutPacket) ; // this needs to be increased every time a new hit is addeded!
